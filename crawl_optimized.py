...
# 구글시트 사이트목록 자동 업데이트
# ─────────────────────────────────────────────
def update_site_in_sheet(gc, site_no: str, updates: dict):
    """
    사이트목록 탭에서 SITE_NO가 일치하는 행을 찾아 지정한 컬럼들을 업데이트
    updates = {'table_body': '...', 'title': '...', 'date': '...', 'URL': '...'}
    """
    if gc is None:
        return False
    try:
        ...
        for i, record in enumerate(records, start=2):  # 2행부터 (1행=헤더)
        ...
        try:
            ...
            while retries < MAX_RETRIES and not success:
                try:
                    ...
                    # ── HTTP 오류 (404/400 등) → URL 탐색 시도 ──
                    if soup is None and status.startswith('http_'):
                        ...
                        if new_url:
                            ...
                            if soup:
                                # 새 URL 구글시트 자동 업데이트
                                if gc:
                                    update_site_in_sheet(gc, str(row.get('SITE_NO', '')), {'URL': new_url})
                                auto_fixed = True
