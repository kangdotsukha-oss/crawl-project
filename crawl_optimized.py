  ...
  "reason": "분석 근거 한 줄 설명"
    ...
    try:
        ...
        if response.status_code != 200:
            logger.error(f"[Claude API 오류] {site_name}: HTTP {response.status_code} - {response.text}")
            return None
        ...
        if 'content' not in result or not result['content']:
            logger.error(f"[Claude API 오류] {site_name}: 응답에 content 없음 - {result}")
            return None

        text = result['content'][0].get('text', '').strip()
        if not text:
            logger.error(f"[Claude API 오류] {site_name}: text 필드 없음")
            return None
        ...
        logger.error(f"[Claude API JSON 파싱 오류] {site_name}: {e} / 응답: {text}")
        return None
    except Exception as e:
