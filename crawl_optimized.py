...
def analyze_with_claude(site_name: str, url: str, html: str) -> dict | None:
    ...
    # HTML 너무 길면 잘라서 전송
    html_trimmed = html[:10000] if len(html) > 10000 else html
  ...
  "reason": "분석 근거 한 줄 설명"
    ...
    try:
        response = requests.post(
            ...
            json={
                "model": "claude-haiku-4-5-20251001",  # 더 저렴한 모델 사용
                "max_tokens": 500,
        ...
        # 응답 전체 로깅 (디버깅용)
        logger.info(f"[Claude API 응답코드] {site_name}: {response.status_code}")

        # 응답 상태 확인
        if response.status_code != 200:
            logger.error(f"[Claude API 오류] {site_name}: HTTP {response.status_code} - {response.text[:200]}")
            return None
        ...
        try:
            ...
            logger.error(f"[Claude API 오류] {site_name}: 응답 JSON 파싱 실패 - {response.text[:200]}")
            return None

        # content 필드 확인
        content_list = result.get('content', [])
        if not content_list:
            logger.error(f"[Claude API 오류] {site_name}: content 없음 - {str(result)[:200]}")
            return None

        text = content_list[0].get('text', '').strip()
        if not text:
            logger.error(f"[Claude API 오류] {site_name}: text 없음")
            return None
        ...
        logger.error(f"[Claude JSON 파싱 오류] {site_name}: {e} / 응답텍스트: {text[:200]}")
        return None
    except Exception as e:
        logger.error(f"[Claude API 오류] {site_name}: {type(e).__name__}: {e}")
        return None
